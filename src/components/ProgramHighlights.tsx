      ...
      <div className="container mx-auto px-4 lg:px-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8 lg:space-y-10">
          <TabsList
            ...
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 rounded-xl bg-muted/80 p-2 backdrop-blur"
          >
            {programs.map((program) => (
              <TabsTrigger
                ...
                value={program.value}
                className={cn(
                  "flex h-full w-full items-center justify-center rounded-xl border px-3 py-2.5 text-center text-sm font-semibold transition-all",
                  "min-h-[70px] sm:min-h-[75px] lg:min-h-[80px]",
              ...
              >
                <span className="text-sm sm:text-base leading-snug text-wrap text-balance">{program.title}</span>
              </TabsTrigger>
            ))}
          </TabsList>
